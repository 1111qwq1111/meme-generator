...
def download_and_extract_repo(repo_name: str, repo_config: Dict[str, str]) -> bool:
    ...
    try:
        ...
        print(f"[SUCCESS] {repo_name} 下载完成")
        return True
        
    except Exception as e:
        print(f"[ERROR] 下载 {repo_name} 失败: {e}")
        return False
    ...
    for repo_name, repo_config in MEME_REPOS.items():
        ...
        if success:
            ...
            if meme_path.exists():
                ...
                print(f"[SUCCESS] 添加meme目录: {meme_path}")
            else:
                print(f"[WARNING] 警告: {repo_name} 中未找到meme目录: {meme_path}")
...
def update_config_file(meme_dirs: List[str]):
    ...
    if not config_file.exists():
        ...
        if example_config.exists():
            ...
            print("[SUCCESS] 从示例文件创建config.toml")
        else:
            print("[ERROR] 未找到配置文件模板")
            return
    ...
    try:
        ...
        print(f"[SUCCESS] 已更新配置文件，添加了 {len(meme_dirs)} 个额外meme目录")
        
    except ImportError:
        print("[ERROR] 需要安装toml库: pip install toml")
    except Exception as e:
        ...
        if result.returncode == 0:
            ...
            if "extra_memes/" not in gitignore_content:
                with open(gitignore_path, 'a', encoding='utf-8') as f:
                    f.write("\n# 额外的meme仓库（自动下载）\nextra_memes/\n")
                print("[SUCCESS] 已将extra_memes/添加到.gitignore")
...
def main():
    ...
    if not Path("meme_generator").exists():
        print("[ERROR] 请在meme-generator项目根目录下运行此脚本")
        sys.exit(1)
